...
Incoming calls and voicemail or park notifications are accepted only from the phone system: the addresses its server name resolves to when CallBridge registers, plus the address that answers the registration. Calls from anywhere else, including other programs on the same PC, are refused with 403. For local development only, setting the environment variable `CALLBRIDGE_ALLOW_LOCAL_TEST_CALLS=1` before starting the desktop app also accepts calls from this PC (loopback); it never opens other addresses. Leave it unset on production installs.
